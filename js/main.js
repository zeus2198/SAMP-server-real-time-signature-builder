...
function operation() {
    ...
    $.ajax({
        ...
        tid = data;
        $("#pop").slideUp('slow');
        $("#pop").html("<div id='popcontent'><span style='color:green;font-family:Gothic;font-size:40px;display:inline-block;'>Done!</span><br><br><span style='font-family:Gothic;font-size:15px;'> The template for yor signature has been created! <br>Template ID : <b>" + data + "</b><br><br>You may enter server information below to generate a link to signarutre<br>" +
            "<br></span></div><input id='sip' style='font-family:Gothic;width:300px;height:30px;' type='text' placeholder='Server IP(eg. 127.0.0.1)' /> <br><br> " +
            "<input id='sport' style='font-family:Gothic;width:300px;height:30px;' type='text' placeholder='Server Port(eg. 7777)' /> <br><br> <button onclick='glink()'>Ok</button> <br><br><div id='gerror' style='display:none;color:#ff0000;font-size:15px;font-family:Gothic;text-align:center;'></div>");
        $("#pop").slideDown('fast');
        $("#popcontent").disableSelection();
    });
